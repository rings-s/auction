<!-- src/lib/components/property/PropertyFeatures.svelte -->
<script>
  ...
  <div class="property-features">
    <div class="space-y-8">
      ...
      {#if features && features.length > 0}
        <div>
          <h3 class="mb-4 text-xl font-bold text-cosmos-text">{$t('properties.property_features')}</h3>
          
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {#each features as feature}
              <div class="flex items-center space-x-2">
                <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary bg-opacity-10">
      ...
      {#if amenities && amenities.length > 0}
        <div>
          <h3 class="mb-4 text-lg font-medium text-cosmos-text">{$t('properties.amenities')}</h3>
          
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {#each amenities as amenity}
              <div class="flex items-center space-x-2">
                <div class="flex h-6 w-6 items-center justify-center rounded-full bg-cosmos-bg-light">
                  <svg class="h-3 w-3 text-cosmos-text" fill="currentColor" viewBox="0 0 20 20">
      ...
      {#if hasValues(outdoorSpaces)}
        <div>
          <h3 class="mb-4 text-lg font-medium text-cosmos-text">{$t('properties.outdoor_spaces')}</h3>
          
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {#each Object.entries(outdoorSpaces) as [key, value]}
              <div>
                <div class="flex items-center">
                  <div class="mr-2 flex h-8 w-8 items-center justify-center rounded-full bg-primary bg-opacity-10">
                    <svg class="h-5 w-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {#if hasValues(parking)}
        <div>
          <h3 class="mb-4 text-lg font-medium text-cosmos-text">{$t('properties.parking')}</h3>
      ...
      {#if hasValues(buildingServices)}
        <div>
          <h3 class="mb-4 text-lg font-medium text-cosmos-text">{$t('properties.building_services')}</h3>
      ...
      {#if hasValues(infrastructure)}
        <div>
          <h3 class="mb-4 text-lg font-medium text-cosmos-text">{$t('properties.infrastructure')}</h3>
