<!-- src/lib/components/ui/ToolTipNew.svelte -->
<script>
  /**
  ...
  function positionTooltip() {
    ...
    // Apply positioning based on the position prop
    switch(position) {
      case 'top':
        tooltipElement.style.bottom = '100%';
        tooltipElement.style.left = '50%';
        tooltipElement.style.transform = `translateX(-50%) translateY(-${offset}px)`;
        break;
      case 'bottom':
      ...
      case 'left':
        ...
        tooltipElement.style.transform = `translateY(-50%) translateX(-${offset}px)`;
        break;
      case 'right':
    ...
    // After applying positioning, get the updated tooltip position
    const updatedTooltipRect = tooltipElement.getBoundingClientRect();
    
    // Now make viewport boundary adjustments
    if (position === 'top' || position === 'bottom') {
      // Horizontal adjustment
      if (updatedTooltipRect.right > viewportWidth) {
        // Too far right
        const rightOverflow = updatedTooltipRect.right - viewportWidth;
        const currentLeftValue = parseFloat(window.getComputedStyle(tooltipElement).left);
        tooltipElement.style.left = `${currentLeftValue - rightOverflow - 5}px`;
      } else if (updatedTooltipRect.left < 0) {
        // Too far left
        const leftOverflow = Math.abs(updatedTooltipRect.left);
        const currentLeftValue = parseFloat(window.getComputedStyle(tooltipElement).left);
        tooltipElement.style.left = `${currentLeftValue + leftOverflow + 5}px`;
      }
    }
    ...
    if (position === 'left' || position === 'right') {
      ...
      if (updatedTooltipRect.bottom > viewportHeight) {
        // Too far down
        const bottomOverflow = updatedTooltipRect.bottom - viewportHeight;
        const currentTopValue = parseFloat(window.getComputedStyle(tooltipElement).top);
        tooltipElement.style.top = `${currentTopValue - bottomOverflow - 5}px`;
      } else if (updatedTooltipRect.top < 0) {
        // Too far up
        const topOverflow = Math.abs(updatedTooltipRect.top);
        const currentTopValue = parseFloat(window.getComputedStyle(tooltipElement).top);
        tooltipElement.style.top = `${currentTopValue + topOverflow + 5}px`;
      }
    }
  }
