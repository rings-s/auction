...
export async function placeBid(auctionId, bidAmount, maxBidAmount = null) {
	try {
		...
		if (errorMessage.includes('not currently accepting bids')) {
			errorMessage =
				'This auction is not currently accepting bids. Please check the auction status and timing.';
...
export async function canAuctionAcceptBids(auctionId) {
	try {
		...
		return {
			...
			reason: statusInfo.is_biddable
				? 'Auction is active'
				: `Auction is ${statusInfo.status_display}`,
			details: statusInfo
		};
	} catch (error) {
		return {
