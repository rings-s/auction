...
async function authenticatedFetch(url, options = {}) {
  ...
  if (!(options.body instanceof FormData) && !finalHeaders['Content-Type'] && !finalHeaders['content-type']) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { data: null, error: { status: response.status, message: `HTTP error: ${response.statusText}` } };
    }
