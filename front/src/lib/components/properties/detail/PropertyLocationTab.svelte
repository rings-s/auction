...
<div class="p-8">
	<h2 class="mb-8 flex items-center text-xl font-bold text-gray-900 dark:text-white">
		<svg
			class="text-primary-500 mr-3 h-6 w-6"
			fill="none"
			stroke="currentColor"
			viewBox="0 0 24 24"
		>
	...
	<div class="grid grid-cols-1 gap-10 md:grid-cols-12">
		...
		<div class="md:col-span-8">
			{#if property.location && (property.location.latitude || property.location.longitude)}
				<div class="relative">
					<div
						...
						class="h-96 w-full overflow-hidden rounded-xl border-2 border-gray-200 bg-gray-100 shadow-lg dark:border-gray-600 dark:bg-gray-700"
					></div>

					<!-- Map Interaction Notice -->
					<div
						class="absolute right-4 bottom-4 max-w-xs rounded-lg bg-white p-3 opacity-90 shadow-lg dark:bg-gray-800"
					>
			...
			{:else}
				<div
					class="flex h-96 w-full items-center justify-center rounded-xl border-2 border-gray-200 bg-gray-100 shadow-lg dark:border-gray-600 dark:bg-gray-700"
				>
					<div class="p-8 text-center">
						...
						>
							<path
								...
								stroke-linejoin="round"
								stroke-width="2"
								d="M9.172 16l-7-7m0 0l7-7m-7 7h18m-6 4h-5m6 4h5m-6 4h-5m-6-7h11m-6 7h6m6-7v-1a1 1 0 00-1-1H8a1 1 0 00-1 1v12a1 1 0 001 1h2.5a1 1 0 001-1V10a1 1 0 00-1-1H6a1 1 0 001-1h2a1 1 0 001 1v7.5a1 1 0 001 1V10a1 1 0 001 1h2a1 1 0 001-1V4a1 1 0 011-1h1z"
							/>
						</svg>
		...
		<div class="md:col-span-4">
			{#if property.location}
				<div
					class="h-full rounded-xl border border-gray-100 bg-white p-6 shadow-lg dark:border-gray-700 dark:bg-gray-800"
				>
					<h3 class="mb-5 flex items-center text-lg font-semibold text-gray-900 dark:text-white">
						<svg
					...
					<div class="space-y-4">
						<div
							class="flex items-center rounded-md px-3 py-2 transition-colors hover:bg-gray-50 dark:hover:bg-gray-700"
						>
							<span class="w-28 flex-shrink-0 font-medium text-gray-600 dark:text-gray-400"
								>{$t('property.address')}:</span
							>
							<span class="ml-2 flex-grow text-gray-900 dark:text-white"
								>{property.location.address}</span
							>
						</div>
						...
						{#if property.location.postal_code}
							...
							>
								<span class="w-28 flex-shrink-0 font-medium text-gray-600 dark:text-gray-400"
									>{$t('property.postalCode')}:</span
								>
								<span class="ml-2 flex-grow text-gray-900 dark:text-white"
									>{property.location.postal_code}</span
								>
							</div>
						{/if}
						...
						>
							<span class="w-28 flex-shrink-0 font-medium text-gray-600 dark:text-gray-400"
								>{$t('property.country')}:</span
							>
							<span class="ml-2 flex-grow text-gray-900 dark:text-white"
								>{property.location.country}</span
							>
						</div>

						{#if property.location.latitude && property.location.longitude}
							<div
								class="mt-1 flex items-center rounded-md border-t border-gray-200 px-2 py-1.5 pt-3 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700"
							>
								<span class="w-28 flex-shrink-0 font-medium text-gray-600 dark:text-gray-400"
									>{$t('property.coordinates')}:</span
								>
								<span class="ml-2 flex-grow font-mono text-sm text-gray-900 dark:text-white">
					...
					<div class="mt-7 border-t border-gray-200 pt-4 dark:border-gray-700">
						<h4 class="mb-3 flex items-center text-base font-medium text-gray-900 dark:text-white">
							<svg
								class="text-primary-500 mr-2 h-4 w-4"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
						...
						<p
							class="dark:bg-gray-750 rounded-lg bg-gray-50 p-3 text-sm text-gray-600 dark:text-gray-400"
						>
