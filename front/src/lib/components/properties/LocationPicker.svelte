...
<script>
	...
	function checkAndInitializeMap() {
		...
		if (!mapElement) {
			...
			if (initAttempts > 10) {
				locationError =
					$t('location.mapInitFailed') ||
					'Unable to initialize map. Please make sure the map section is visible.';
			}
			return false;
		}
		// Only initialize if the element is visible
		if (
			mapElement.offsetParent === null ||
		...
		if (
			...
			!isElementInViewport(mapElement)
		) {
			// Removed console.log for production
			// console.error('Map element is not visible, retrying later...');
			return false;
	...
	async function detectLocation() {
		if (!navigator.geolocation) {
			locationError =
				$t('location.geolocationNotSupported') || 'Geolocation is not supported by your browser.';
			return;
		}
		...
		try {
			...
			if (error.code === 1) {
				...
				locationError =
					...
					'Unable to determine your location. Try again or enter your location manually.';
			} else if (error.code === 3) {
				locationError = $t('location.timeout') || 'Location request timed out. Please try again.';
			} else {
				locationError =
					$t('location.detectionFailed') ||
					'Failed to detect location: ' + (error.message || 'Unknown error');
			}
		} finally {
	...
	async function reverseGeocode(lat, lng) {
		try {
			...
			locationError =
				$t('location.geocodingFailed') || 'Failed to get address details from coordinates';
...
<div class="space-y-6">
	...
	<div class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-6">
		...
		<div class="sm:col-span-6">
			<label
				for="address"
				class="required block text-sm font-medium text-gray-700 dark:text-gray-300"
			>
		...
		<div class="sm:col-span-3">
			<label for="city" class="required block text-sm font-medium text-gray-700 dark:text-gray-300">
		...
		<div class="sm:col-span-3">
			...
			<div class="mt-1">
				<input
					...
					bind:value={country}
					readonly
					class="focus:ring-primary-500 focus:border-primary-500 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm sm:text-sm dark:border-gray-700 dark:bg-gray-700 dark:bg-gray-800 dark:text-white"
				/>
			</div>
		</div>
	</div>
					...
					<div>
						...
						<p class="text-xs text-gray-500 dark:text-gray-400">
							{$t('location.detectHelp') ||
								'Use your device location to automatically fill address fields'}
						</p>
					</div>
					<button
						type="button"
						...
						class="bg-primary-600 hover:bg-primary-700 focus:ring-primary-500 inline-flex items-center rounded border border-transparent px-3 py-1.5 text-xs font-medium text-white shadow-sm focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:opacity-50"
					>
						{#if locationLoading}
							<svg
