...
<script>
  ...
  function getButtonClasses() {
    const baseClasses = 'relative transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500';
    const variantClasses = {
...
>
  ...
  {#if supportsNativeShare && mounted}
    ...
    >
      <svg class={iconSizes[variant]} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    {#if platformConfig[platform]}
      ...
      >
        ...
        {#if config.icon === 'facebook'}
          <svg class="{iconSizes[variant]}" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  {#if showTooltip}
    <div
      class="absolute {orientation === 'vertical' ? 'left-full ml-2' : 'bottom-full mb-2'} 
        left-1/2 transform -translate-x-1/2 px-3 py-2 
        bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 
        text-sm rounded-lg shadow-lg whitespace-nowrap z-50"
      role="tooltip"
      transition:fly={{ y: -10, duration: 200 }}
    >
      {tooltipMessage}
      <div class="absolute {orientation === 'vertical' ? 'left-0 top-1/2 transform -translate-y-1/2 -translate-x-1 w-2 h-2 rotate-45' : 'top-full left-1/2 transform -translate-x-1/2 w-2 h-2 rotate-45'} 
