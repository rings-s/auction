...
<div class="space-y-4">
  ...
  <div 
    class="relative border-2 border-dashed p-6 rounded-lg text-center
      {dragActive ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-neutral-300 dark:border-neutral-600'}
  ...
  >
    ...
    {:else}
      <div class="text-center">
        ...
        <button
          type="button"
          class="mt-2 inline-flex items-center px-4 py-2 border border-neutral-300 rounded-md shadow-sm text-sm font-medium text-neutral-700 bg-white hover:bg-neutral-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          on:click={() => fileInput.click()} disabled={disabled || uploading}
          >
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {#each files as file, index}
          <div class="relative group">
            <div class="aspect-w-3 aspect-h-2 rounded-lg overflow-hidden bg-neutral-100 dark:bg-neutral-800">
              {#if file.type.startsWith('image/')}
                <img
            ...
            <button
              type="button"
              class="absolute top-2 right-2 p-1 rounded-full bg-danger-100 text-danger-600 opacity-0 group-hover:opacity-100 transition-opacity"
