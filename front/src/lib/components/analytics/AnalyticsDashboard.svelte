...
<script>
	...
	const icons = {
		home: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"></path></svg>`,
		...
		briefcase: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6 6V5a3 3 0 013-3h2a3 3 0 013 3v1h2a2 2 0 012 2v3.57A22.952 22.952 0 0110 13a22.95 22.95 0 01-8-1.43V8a2 2 0 012-2h2zm2-1a1 1 0 011-1h2a1 1 0 011 1v1H8V5zm1 5a1 1 0 011-1h.01a1 1 0 110 2H10a1 1 0 01-1-1z" clip-rule="evenodd"></path><path d="M2 13.692V16a2 2 0 002 2h12a2 2 0 002-2v-2.308A24.974 24.974 0 0110 15c-2.796 0-5.487-.46-8-1.308z"></path></svg>`,
		cash: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg>`,
		trendUp: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z" clip-rule="evenodd"></path></svg>`
	};
		...
		try {
			...
			if (propertyChartRef && chartData?.properties) {
				await Plotly.newPlot(
					propertyChartRef,
					chartData.properties.data,
					chartData.properties.layout,
					chartData.properties.config
				);
			}
			...
			if (paymentChartRef && chartData?.payments) {
				await Plotly.newPlot(
					paymentChartRef,
					chartData.payments.data,
					chartData.payments.layout,
					chartData.payments.config
				);
			}
			...
			if (workerChartRef && chartData?.workers) {
				await Plotly.newPlot(
					workerChartRef,
					chartData.workers.data,
					chartData.workers.layout,
					chartData.workers.config
				);
			}
			...
			if (heatMapRef && chartData?.heatMap) {
				await Plotly.newPlot(
					...
					chartData.heatMap.layout,
					chartData.heatMap.config
				);
			}
		} catch (err) {
...
<div class="space-y-6">
	...
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
		...
		<div class="mt-4 flex space-x-3 sm:mt-0">
			<Button onClick={refreshData} variant="outline" loading={refreshing}>
	...
	{:else if analytics}
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
			...
			{#key analytics.totalProperties}
				{@const propertyGrowth = getGrowthIndicator(
					analytics.totalProperties,
					analytics.previousProperties
				)}
				<StatCard
			...
			{#key analytics.activeWorkers}
				{@const workerGrowth = getGrowthIndicator(
					analytics.activeWorkers,
					analytics.previousWorkers
				)}
				<StatCard
			...
			<StatCard
				...
				color={analytics.occupancyRate >= 80
					? 'success'
			...
			<StatCard
				...
				color={analytics.workerUtilization >= 80
					? 'success'
					: analytics.workerUtilization >= 60
						? 'warning'
						: 'error'}
				href="/dashboard/workers"
			/>
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
			...
			{#if (selectedChart === 'overview' || selectedChart === 'properties') && propertyAnalytics}
				...
				>
					<div class="mb-4 flex items-center justify-between">
						...
						<Button href="/dashboard/properties" variant="outline" size="compact">
			...
			{#if (selectedChart === 'overview' || selectedChart === 'heatmap') && heatMapData}
				<div
					class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm lg:col-span-2 dark:border-gray-700 dark:bg-gray-800"
				>
					<div class="mb-4 flex items-center justify-between">
						<h3 class="text-lg font-medium text-gray-900 dark:text-white">
			...
			>
				...
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{#each analytics.insights as insight}
						<div
							class="rounded-md p-4 bg-{insight.type === 'positive'
								? 'green'
								: insight.type === 'warning'
									? 'yellow'
									: 'blue'}-50 dark:bg-{insight.type === 'positive'
								? 'green'
								: insight.type === 'warning'
									? 'yellow'
									: 'blue'}-900/20"
						>
							<div class="flex items-start">
								...
								<div class="ml-3">
									...
									{#if insight.action}
										<Button href={insight.actionUrl} variant="outline" size="compact" class="mt-2">
											{insight.action}
										</Button>
									{/if}
								</div>
