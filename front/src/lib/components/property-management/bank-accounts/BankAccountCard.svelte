...
<script>
	...
	let statusText = $derived(
		account.is_verified ? $t('bankAccount.verified') : $t('bankAccount.pending')
	);
...
>
	...
	{#if account.is_primary}
		<div class="absolute {compact ? 'top-2 right-2' : 'top-4 right-4'}">
			<span
				class="bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-200 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
			>
	...
	<div class="space-y-{compact ? '3' : '4'}">
		...
		<div class="space-y-{compact ? '2' : '3'}">
			...
			<div>
				...
				<p
					class="mt-0.5 font-mono text-{compact
						? 'xs'
						: 'sm'} break-all text-gray-900 dark:text-white"
				>
		...
		{#if showActions}
			<div
				class="flex space-x-2 pt-{compact
					? '3'
					: '4'} border-t border-gray-200 dark:border-gray-700"
			>
				<!-- Edit Button -->
				<Button
					onClick={handleEdit}
				...
				<Button
					...
					variant="outline"
					size={compact ? 'compact' : 'small'}
					class="text-red-600 hover:border-red-300 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
				>
