...
<script>
	...
	let availableWorkers = $derived(workers.filter(w => w.is_available && w.status === 'active').length);
	...
	let topPerformers = $derived(() => {
		return workers
			...
			.sort((a, b) => parseFloat(b.performance.completionRate) - parseFloat(a.performance.completionRate))
			.slice(0, 5);
	});
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
			...
			<div class="flex items-end">
				<p class="text-sm text-gray-600 dark:text-gray-400">
					{$t('common.showing')} {filteredWorkers.length} {$t('common.of')} {totalWorkers} {$t('worker.workers')}
				</p>
			</div>
		</div>
	</div>
	...
	{:else}
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
			...
			<StatCard
				...
				value={activeWorkers}
				icon='<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path></svg>'
				color="success"
			/>
			...
			<StatCard
				...
				color={parseFloat(utilizationRate) >= 80 ? 'success' : parseFloat(utilizationRate) >= 60 ? 'warning' : 'error'}
			/>
			...
			<StatCard
				...
				color={parseFloat(averageRating) >= 4 ? 'success' : parseFloat(averageRating) >= 3 ? 'warning' : 'error'}
			/>
		</div>
	{/if}
							...
							<div class="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-md">
								...
								<div class="text-right">
									...
									<p class="text-xs text-gray-500">
										{worker.performance.totalJobs} {$t('worker.jobs')}
									</p>
								</div>
							</div>
						{/each}
					...
					<div class="space-y-4">
						{#each workersNeedingAttention as worker}
							<div class="flex items-center justify-between p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-md">
								<div class="flex items-center space-x-3">
									<div class="flex-shrink-0">
										<div class="h-8 w-8 rounded-full bg-yellow-100 dark:bg-yellow-800 flex items-center justify-center">
								...
								<div class="text-right">
									<p class="text-sm font-medium text-{getPerformanceColor(worker.performance.completionRate)}-600">
										{worker.performance.completionRate}%
									</p>
									<p class="text-xs text-gray-500">
						...
						<div class="p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
							<div class="flex items-center justify-between mb-2">
								...
								<span class="text-xs text-gray-500">
									{categoryWorkers.length} {$t('worker.workers')}
								</span>
							</div>
								...
								{#if category.hourly_rate_min && category.hourly_rate_max}
									<div class="flex justify-between text-sm">
										...
										<span class="text-gray-900 dark:text-white">
											{formatHourlyRate(category.hourly_rate_min)} - {formatHourlyRate(category.hourly_rate_max)}
										</span>
									</div>
								{/if}
								
								{#if categoryWorkers.length > 0}
									{@const avgRating = categoryWorkers
										.filter(w => w.rating)
						...
						<tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
							{#each filteredWorkers.slice(0, 10) as worker}
								<tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
									...
									<td class="px-6 py-4 whitespace-nowrap">
										<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{getWorkerStatusColor(worker.status)}-100 text-{getWorkerStatusColor(worker.status)}-800 dark:bg-{getWorkerStatusColor(worker.status)}-900 dark:text-{getWorkerStatusColor(worker.status)}-200">
											{worker.status}
										</span>
									</td>
									<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
