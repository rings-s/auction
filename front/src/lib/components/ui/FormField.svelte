...
<script>
	...
	const baseInputStyle =
		'shadow-sm block w-full sm:text-sm rounded-md bg-white dark:bg-gray-50 text-gray-900 dark:text-gray-800 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-primary-500 focus:border-primary-500';
...
<div>
		...
		<div class="relative">
			...
			{#if error}
				<div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
					<svg class="text-danger-500 h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
						<path
							fill-rule="evenodd"
	...
	{:else if type === 'currency'}
		<div class="relative">
			<div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
				<span class="text-gray-500 sm:text-sm dark:text-gray-400">{currencySymbol}</span>
			</div>
			<input
				{id}
				type="number"
	...
	{#if error}
		<p class="text-danger-600 dark:text-danger-500 mt-1 text-sm">{error}</p>
	{:else if helpText}
