...
<nav
	class="scrollbar-none relative flex w-full items-center overflow-x-auto px-1 py-3 {class_}"
	...
	<ol class="inline-flex w-full flex-nowrap items-center {spacing}">
		{#each items as item, index (item.href)}
			<li class="flex items-center whitespace-nowrap">
				...
				{#if item.active}
					<span class="{textSize} font-medium text-gray-800 dark:text-gray-200" aria-current="page">
						{item.label}
					</span>
				{:else}
					<a
						href={item.href}
						class="{textSize} group text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 focus:ring-primary-500 relative rounded-md px-1 py-0.5 font-medium transition-colors duration-200 focus:ring-2 focus:ring-offset-2 focus:outline-none"
					>
						<span class="relative z-10">{item.label}</span>
						<span
							class="bg-primary-500 absolute bottom-0 left-0 h-0.5 w-0 transition-all duration-200 group-hover:w-full"
						></span>
					</a>
				{/if}
			</li>
