...
<div class="rounded-lg border border-gray-200 bg-white {compact ? 'p-3' : 'p-4'}">
		...
		<div class="space-y-3">
			{#each Array(5) as _}
				<div class="flex animate-pulse items-start space-x-3">
					<div class="h-8 w-8 flex-shrink-0 rounded-full bg-gray-200"></div>
		...
		<div class="space-y-3">
			{#each displayActivities as activity (activity.id || `${activity.timestamp}-${activity.title}`)}
				<div class="flex items-start space-x-3">
					<div
						class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full {getPriorityColor(
							activity.priority
						)}"
					>
