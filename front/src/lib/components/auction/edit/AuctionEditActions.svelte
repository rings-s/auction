    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      {#if canDelete}
        <div class="flex items-center">
          <div class="border-l border-neutral-200 dark:border-neutral-700 pl-4 ml-4">
            <Button 
              variant="danger" 
              size="default"
              on:click={onDelete} 
    ...
    {#if auction && (auction.status === 'live' || auction.status === 'scheduled')}
      <div class="mt-4 p-3 bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800 rounded-lg">
        <div class="flex items-start">
          <svg class="h-5 w-5 text-warning-500 mt-0.5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
