...
<script>
	...
	async function handleMediaUpload(e) {
		...
		try {
			...
			if (!isEditing) {
				...
				const uploadedMedia = await uploadMultipleMedia(files, 'auction', auction.id, {
	...
	export async function uploadTempMedia(auctionId) {
		...
		try {
			...
			const uploadedMedia = await uploadMultipleMedia(files, 'auction', auctionId, {
	...
	export function validateForm() {
		...
		if (!auction.title) {
			return {
				valid: false,
				error: safeTranslate('auction.titleRequired', 'Title is required'),
				tab: 'basic'
			};
		}
		...
		if (!auction.related_property_id) {
			return {
				valid: false,
				error: safeTranslate('auction.propertyRequired', 'Property is required'),
				tab: 'property'
			};
		}
		...
		if (!auction.start_date || !auction.end_date) {
			return {
				valid: false,
				error: safeTranslate('auction.datesRequired', 'Start and end dates are required'),
				tab: 'scheduling'
			};
		}
		...
		if (endDate <= startDate) {
			return {
				valid: false,
				error: safeTranslate('auction.endDateAfterStart', 'End date must be after start date'),
				tab: 'scheduling'
			};
		}
		...
		if (auction.registration_deadline) {
			...
			if (regDeadline >= startDate) {
				return {
					valid: false,
					error: safeTranslate(
						'auction.regDeadlineBeforeStart',
		...
		if (!auction.starting_bid || parseFloat(auction.starting_bid) <= 0) {
			return {
				valid: false,
				error: safeTranslate('auction.startingBidRequired', 'Starting bid is required'),
				tab: 'financial'
			};
		}
	...
	export function prepareDataForSubmission() {
		...
		if (preparedData.registration_deadline) {
			preparedData.registration_deadline = new Date(
				preparedData.registration_deadline
			).toISOString();
		}
...
<div class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
	...
	<div class="bg-gray-50 px-6 py-4 dark:bg-gray-900">
		<div class="flex items-center justify-between">
			<h2 class="text-lg font-medium text-gray-900 dark:text-white">
				{isEditing
					? safeTranslate('auction.editAuction', 'Edit Auction')
					: safeTranslate('auction.createAuction', 'Create Auction')}
			</h2>
			<div class="text-sm text-gray-500 dark:text-gray-400">
				{safeTranslate('common.step', 'Step')}
				{currentStep}
				{safeTranslate('common.of', 'of')}
				{totalSteps}
			</div>
		</div>
	...
	<!-- Tabs Navigation -->
	<div class="border-b border-gray-200 px-6 pt-6 pb-3 dark:border-gray-700">
		<Tabs {tabs} bind:activeTab iconPosition="left" variant="underline" size="default" />
	</div>
				...
				<div class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-6">
					...
					<div class="sm:col-span-4">
						<FormField
							...
							required={true}
							error={touched.title && !auction.title
								? safeTranslate('auction.titleRequired', 'Title is required')
								: ''}
					...
					<div class="sm:col-span-6">
						<fieldset class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
							<legend class="px-2 text-base font-medium text-gray-900 dark:text-white">
							...
							<div
								...
								role="radiogroup"
								aria-labelledby="auction-type-legend"
							>
								{#each [{ value: 'sealed', label: safeTranslate('auction.typeSealed', 'Sealed Bid'), desc: safeTranslate('auction.typeSealedDesc', 'Sealed bid auction') }, { value: 'reserve', label: safeTranslate('auction.typeReserve', 'Reserve'), desc: safeTranslate('auction.typeReserveDesc', 'Auction with reserve price') }, { value: 'no_reserve', label: safeTranslate('auction.typeNoReserve', 'No Reserve'), desc: safeTranslate('auction.typeNoReserveDesc', 'Auction without reserve') }] as type}
									<button
										type="button"
										role="radio"
										aria-checked={auction.auction_type === type.value}
										class="focus:ring-primary-500 relative rounded-lg border p-4 text-left transition-all duration-200 focus:ring-2 focus:ring-offset-2 focus:outline-none"
									...
									>
										{#if auction.auction_type === type.value}
											<div class="text-primary-500 dark:text-primary-400 absolute top-2 right-2">
							...
							<div class="space-y-4">
								<Switch
									...
									description={safeTranslate(
										'auction.publishedHelp',
										'Make this auction visible to the public'
									)}
				...
				<div class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
					...
					<div>
						<FormField
							...
							required={true}
							error={touched.start_date && !auction.start_date
								? safeTranslate('auction.startDateRequired', 'Start date is required')
								: ''}
					...
					<div>
						<FormField
							...
							required={true}
							error={touched.end_date && !auction.end_date
								? safeTranslate('auction.endDateRequired', 'End date is required')
								: ''}
					...
					<div>
						<FormField
							...
							helpText={safeTranslate(
								'auction.registrationDeadlineHelp',
								'Optional deadline for registration'
							)}
						/>
					</div>
				...
				<div class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
					...
					<div>
						<FormField
							...
							required={true}
							error={touched.starting_bid && !auction.starting_bid
								? safeTranslate('auction.startingBidRequired', 'Starting bid is required')
								: ''}
					...
					<div class="sm:col-span-2">
						<FormField
							...
							step="1"
							helpText={safeTranslate(
								'auction.minimumParticipantsHelp',
		...
		{:else if activeTab === 'property'}
			...
			<div class="space-y-6">
				<PropertySelector
					...
					error={touched.related_property_id && !auction.related_property_id
						? safeTranslate('auction.propertyRequired', 'Property is required')
						: ''}
				/>
			</div>
						...
						>
							<div class="flex items-center justify-between">
								...
								<span class="text-sm text-gray-500 dark:text-gray-400">
									...
									{safeTranslate('auction.items', 'items')}
								</span>
							</div>
						</div>
						<div class="p-6">
							<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
								{#each auction.media as media (media.id)}
									...
									>
										...
										<div class="bg-white p-2 dark:bg-gray-800">
										...
										<!-- Controls -->
										<div
											class="bg-opacity-50 absolute inset-0 flex items-center justify-center bg-gray-900 opacity-0 transition-opacity duration-200 group-hover:opacity-100"
										>
											<div class="flex flex-col space-y-2">
												<button
													type="button"
													class="rounded bg-white px-3 py-1 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-50"
													on:click={() => setPrimaryMedia(media.id)}
													aria-label={safeTranslate(
														'auction.setPrimaryMedia',
														'Set as primary media for {name}',
														{ name: media.name }
													)}
												>
												...
												<button
													type="button"
													class="bg-danger-500 hover:bg-danger-600 rounded px-3 py-1 text-xs font-medium text-white transition-colors"
													on:click={() => removeMedia(media.id)}
													aria-label={safeTranslate('auction.removeMedia', 'Remove media {name}', {
														name: media.name
													})}
												>
										...
										{#if media.is_primary}
											<div class="absolute top-0 left-0 m-2">
												<span
													class="bg-success-100 text-success-800 dark:bg-success-900 dark:text-success-200 inline-flex items-center rounded-full px-1.5 py-0.5 text-xs font-medium"
												>
				...
				<div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
					...
					>
						...
						<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
							{safeTranslate(
								'auction.termsConditionsHelp',
								'Define the terms and conditions for this auction'
							)}
						</p>
					</div>
					<div class="p-6">
	...
	>
		<button
			type="button"
			class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
		...
		{#if currentStep < totalSteps}
			<button
				type="button"
				class="bg-primary-600 hover:bg-primary-700 inline-flex items-center rounded-md border border-transparent px-3 py-2 text-sm font-medium text-white shadow-sm transition-colors"
