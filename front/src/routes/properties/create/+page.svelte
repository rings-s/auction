...
<script>
	...
	async function handleSubmit() {
		...
		try {
			...
			if (mediaFiles.length > 0 && createdPropertyId) {
				...
				try {
					await uploadPropertyMediaBatch(createdPropertyId, mediaFiles, (completed, total) => {
	...
	$: isAuthorized =
		$user && ($user.role === 'owner' || $user.role === 'appraiser' || $user.is_staff);
</script>
	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		...
		{:else}
			...
			<div class="mx-auto max-w-4xl">
				<!-- Header Component -->
				<PropertyCreateHeader {currentStep} {totalSteps} {loading} />
