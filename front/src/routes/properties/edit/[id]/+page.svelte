...
<script>
    ...
    import { getProperty, updateProperty, uploadPropertyMediaBatch, deletePropertyMedia, updatePropertyRoom, addPropertyRoom, deletePropertyRoom } from '$lib/api/property';
    ...
    async function handleSave() {
      ...
      try {
        ...
        const currentRoomIds = rooms.filter(r => r.id && !r.id.toString().startsWith('temp-')).map(r => r.id);
  ...
  <svelte:head>
    <title>{property?.title ? `${$t('property.edit')} - ${property.title}` : $t('property.editProperty')}</title>
  </svelte:head>
