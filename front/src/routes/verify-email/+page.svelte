...
<script>
  ...
  async function handleVerify(event) {
    ...
    debugLog('handleVerify called with:', { email: submittedEmail, codeLength: verificationCode?.length });
  ...
  <div class="w-full max-w-md space-y-8 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg transform transition-all">
    <div>
      <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-primary-100 dark:bg-primary-900">
        <svg class="h-8 w-8 text-primary-600 dark:text-primary-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    <!-- Error Alert -->
    {#if error}
      <div class="rounded-lg bg-red-50 dark:bg-red-900/30 p-4 border-l-4 border-red-500 dark:border-red-600" role="alert">
    ...
    <!-- Success Alert -->
    {#if resendSuccess}
      <div class="rounded-lg bg-green-50 dark:bg-green-900/30 p-4 border-l-4 border-green-500 dark:border-green-600" role="alert">
        <div class="flex items-start">
          <div class="flex-shrink-0">
    ...
    {#if DEBUG}
      <details class="mt-4 text-xs text-gray-500 dark:text-gray-400">
        <summary class="cursor-pointer hover:text-gray-700 dark:hover:text-gray-300">Debug Info</summary>
